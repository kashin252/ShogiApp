...
### Logic Verification
- Checked `useGame.ts`:
  - `makeMove` now resets `senteTime` or `goteTime` to `settings.timeControl` immediately after a move is processed.
  - Fischer increment logic (`prev + 5`) has been removed.
  - `undo` logic correctly resets the clock to the selected time limit, allowing for a fresh start on the reconsidered move.
